...
- ✅ **双风格提示词**: 简洁型(150-250字) / 详细型(600-1000字) 切换
...
- **参考**: 朗诺示例文档
- **适用**: 高质量，精细控制
- **提示词结构**: 多段落 + 详细排版说明 + 质感光影

### 切换方式

在风格配置页面中选择 `promptStyle`:
- `concise` - 简洁型（默认）
- `detailed` - 详细型
...
1. **文案优化层** (`copyOptimizer.ts`)
   - 根据海报类型优化文案表现力
   - 支持 7 种优化策略（极简聚焦/场景化包装/数据化表达等）

2. **版式配置层** (`layoutBuilder.ts`)
   - 生成详细的版式配置
   - 支持 Logo、标题、副标题、卖点的精确布局

3. **提示词模板层** (`concisePromptGenerator.ts` / `detailedPromptGenerator.ts`)
   - 简洁型：密集精准的单段落提示词
   - 详细型：多段落的超详细描述
